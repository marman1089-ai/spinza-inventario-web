...
def _sales_dashboard_data(cur, active_store: str):
    ...
    total_net = scalar(f"SELECT COALESCE(SUM(net_amount),0) AS v FROM sales_entries{where_sql}", params)
    total_orders = scalar(f"SELECT COALESCE(SUM(order_count),0) AS v FROM sales_entries{where_sql}", params)
    days_recorded = scalar(f"SELECT COUNT(DISTINCT sale_date) AS v FROM sales_entries{where_sql}", params)
    ...
    month_rows = cur.execute(
        ...
        SELECT SUBSTR(sale_date, 1, 7) AS ym,
               COUNT(DISTINCT sale_date) AS recorded_days,
               COALESCE(SUM(net_amount),0) AS total_net
        FROM sales_entries
        {(where_sql + ' AND ' if where_sql else ' WHERE ')} sale_date LIKE {ph}
        GROUP BY SUBSTR(sale_date, 1, 7)
        ORDER BY ym
        """,
        tuple(params + [f"{current_year}-%"]),
    ).fetchall()

    month_map = {r.get("ym"): {"recorded_days": int(r.get("recorded_days") or 0), "total_net": float(r.get("total_net") or 0)} for r in month_rows}
    month_labels = [
    ...
    for idx, label in enumerate(month_labels, start=1):
        ym = f"{current_year}-{idx:02d}"
        info = month_map.get(ym, {})
        months.append({
            "label": label,
            "ym": ym,
            "active": bool(info.get("recorded_days")),
            "recorded_days": int(info.get("recorded_days") or 0),
            "total_net": float(info.get("total_net") or 0),
            "is_current": ym == today[:7],
        })
...
def gestionale_entrate_get(request: Request, sale_date: str = "", sales_channel: str = "ALL"):
    ...
    with connect() as conn:
        cur = conn.cursor()
        rows = cur.execute(f"SELECT * FROM sales_entries{where_sql} ORDER BY id DESC LIMIT 100", tuple(params)).fetchall()
        ...
        summary_row = cur.execute(
            f"SELECT COUNT(*) AS entry_count, COUNT(DISTINCT sale_date) AS day_count, COALESCE(SUM(net_amount),0) AS total_net, COALESCE(SUM(order_count),0) AS total_orders FROM sales_entries{where_sql}",
            tuple(params),
        ).fetchone()
        latest_entry = cur.execute(f"SELECT * FROM sales_entries{where_sql} ORDER BY id DESC LIMIT 1", tuple(params)).fetchone()
    channels = [r.get("sales_channel") for r in channel_rows if (r.get("sales_channel") or '').strip()]
    summary = {
        "entry_count": int((summary_row or {}).get("entry_count") or 0),
        "day_count": int((summary_row or {}).get("day_count") or 0),
        "total_net": float((summary_row or {}).get("total_net") or 0),
        "total_orders": int((summary_row or {}).get("total_orders") or 0),
    }
    summary["avg_ticket"] = (summary["total_net"] / summary["total_orders"]) if summary["total_orders"] else 0
    return render("gestionale_entrate.html", user=user, brand=brand, active_store=active_store, current_module="gestionale", rows=rows, channels=channels, sale_date=sale_date, selected_channel=sales_channel, today=_today_str(), summary=summary, latest_entry=latest_entry)
