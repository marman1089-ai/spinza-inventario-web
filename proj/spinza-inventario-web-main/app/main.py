...
def _annotate_sales_tree_month_share(nodes, month_amount: float, month_quantity: float, parent_name: str = ''):
    for n in nodes or []:
        total_amount = float(n.get('total_amount') or n.get('amount') or 0)
        total_quantity = float(n.get('total_quantity') or n.get('quantity') or 0)
        n['display_amount'] = total_amount
        n['display_quantity'] = total_quantity
        n['month_share_amount'] = round((total_amount / month_amount) * 100.0, 1) if month_amount > 0 else 0.0
        n['month_share_quantity'] = round((total_quantity / month_quantity) * 100.0, 1) if month_quantity > 0 else 0.0
        n['parent_name'] = parent_name or ''
        _annotate_sales_tree_month_share(n.get('children') or [], month_amount, month_quantity, n.get('name') or '')
    return nodes
...
SALES_REPORT_AUTO_GROUP_BY_NAME = {
    ...
    'buona cavolo': 'Pinze stagionali',
    'che cavolo': 'Pinze stagionali',
    'miele on tartufo': 'Pinze speciali',
    'nordica': 'Pinze stagionali',
...
SALES_REPORT_AUTO_GROUP_KEYWORDS = [
    ('Delivery', [' delivery', 'delivery ', 'delivery', 'deliveroo', 'glovo', 'just eat']),
    ('Soft Drink', ['acqua', 'coca cola', 'cola', 'fanta', 'sprite', 'limonata', 'chinotto', 'gazzosa', 'tonica', 'tonic', 'succo', 'tea', 'tè']),
    ('Alcol Drink', ['birra', 'vino', 'spritz', 'cocktail', 'aperol', 'campari', 'prosecco', 'gin', 'rum', 'whisky', 'whiskey', 'vodka', 'amaro']),
    ('Desert', ['tiramisu', 'stiramisu', 'dessert', 'desert', 'dolce', 'cheesecake', 'brownie', 'cookie', 'gelato']),
    ('Pinze classiche', ['margherita', 'marinara', 'diavola', 'nuda', 'bufala', 'romana', 'napoli']),
    ('Pinze speciali', ['porchetta', 'speck', 'tartufo', 'salsiccia', 'nduja', 'nduja', 'burrata', 'mortadella', 'crudo']),
    ('Pinze stagionali', ['zucca', 'porcini', 'fragolina', 'asparagus', 'asparago', 'carciofo', 'fave', 'pears', 'finocchio', 'cavolo', 'nordica']),
]
...
def _sales_report_guess_group(name: str) -> str:
    norm = _sales_name_norm(name)
    if not norm:
        return ''
    if norm in SALES_REPORT_AUTO_GROUP_BY_NAME:
        return SALES_REPORT_AUTO_GROUP_BY_NAME[norm]
    for group_name, keywords in SALES_REPORT_AUTO_GROUP_KEYWORDS:
...
def _sales_report_tree(cur, period_id: int):
    ...
    def subtree_totals(node_id: int):
        amount_total = 0.0
        quantity_total = 0.0
        for item in by_parent.get(node_id, []):
            own_amount = float(item.get('amount') or 0)
            own_quantity = float(item.get('quantity') or 0)
            child_amount, child_quantity = subtree_totals(int(item['id']))
            amount_total += own_amount + child_amount
            quantity_total += own_quantity + child_quantity
        return amount_total, quantity_total
    ...
    def build(parent_id=None, sibling_totals=None):
        ...
        for idx, child in enumerate(children, start=1):
            ...
            node['share_amount'] = round((total_amount / float(totals.get('amount') or 0)) * 100.0, 1) if float(totals.get('amount') or 0) > 0 else 0.0
            node['share_quantity'] = round((total_quantity / float(totals.get('quantity') or 0)) * 100.0, 1) if float(totals.get('quantity') or 0) > 0 else 0.0
    ...
    for child in root_children:
        own_amount = float(child.get('amount') or 0)
        own_quantity = float(child.get('quantity') or 0)
        sub_amount, sub_quantity = subtree_totals(int(child['id']))
        root_totals['amount'] += own_amount + sub_amount
        root_totals['quantity'] += own_quantity + sub_quantity
...
def _sales_report_state(request: Request, user: dict | None, month_key: str = '', store: str = ''):
    brand = _current_store_scope(request, user)
    active_store = request.session.get('active_store') if is_admin(request) else None
    selected_store = (store or request.session.get('sales_report_store') or (brand if brand != 'ALL' else (active_store or 'spinza'))).strip()
    if selected_store not in STORES:
        selected_store = brand if brand != 'ALL' else (active_store or 'spinza' or 'spinza')
    if not is_admin(request):
        selected_store = (user or {}).get('store') or selected_store
    selected_month = _month_key_from_value(month_key or request.session.get('sales_report_month_key') or '')
    request.session['sales_report_store'] = selected_store
    request.session['sales_report_month_key'] = selected_month
    return selected_store, selected_month, brand, active_store


def _sales_report_redirect_url(store: str, month_key: str, **params):
    q = [f"store={store}", f"month_key={month_key}"]
    for key, value in params.items():
        if value is None or value == '':
            continue
        q.append(f"{key}={value}")
    return '/gestionale/report-vendite?' + '&'.join(q)
...
def sales_report_page(request: Request, month_key: str = '', store: str = '', import_ok: int = 0, import_error: str = ''):
    ...
    selected_store, selected_month, brand, active_store = _sales_report_state(request, user, month_key, store)
    ...
    with connect() as conn:
        ...
        top_amount = sum(float(x.get('total_amount') or x.get('amount') or 0) for x in tree)
        top_qty = sum(float(x.get('total_quantity') or x.get('quantity') or 0) for x in tree)
        tree = _annotate_sales_tree_month_share(tree, top_amount, top_qty)
    ...
    return render(
        ...
        tree=tree,
        chart_tree_json=json.dumps(tree, ensure_ascii=False),
...
async def sales_report_clear_month(request: Request):
    ...
    selected_store, selected_month, _, _ = _sales_report_state(request, user, str(form.get('month_key') or ''), str(form.get('store') or ''))
    with connect() as conn:
        cur = conn.cursor()
        period_id = _ensure_sales_report_period(cur, selected_store, selected_month, user.get('username') or 'system')
        cur.execute(f"DELETE FROM sales_report_groups WHERE period_id={_ph()}", (period_id,))
        _ensure_sales_report_groups_from_models(cur, period_id, selected_store, user.get('username') or 'system')
        _log(cur, store=selected_store, username=user['username'], action='DELETE', category='REPORT_VENDITE', name=f'Svuotato mese report vendite {selected_month}', delta=0)
        conn.commit()
    return RedirectResponse(_sales_report_redirect_url(selected_store, selected_month), status_code=HTTP_303_SEE_OTHER)
...
async def sales_report_import_file(request: Request, upload_file: UploadFile = File(...)):
    ...
    form = await request.form()
    store, month_key, _, _ = _sales_report_state(request, user, str(form.get('month_key') or ''), str(form.get('store') or ''))
    ...
    if not raw:
        return RedirectResponse(_sales_report_redirect_url(store, month_key, import_error='File+vuoto'), status_code=HTTP_303_SEE_OTHER)

    try:
        rows = _parse_sales_report_upload(filename, raw)
    except Exception as e:
        msg = str(e).replace(' ', '+')
        return RedirectResponse(_sales_report_redirect_url(store, month_key, import_error=msg), status_code=HTTP_303_SEE_OTHER)
    ...
    with connect() as conn:
        ...
        for row in rows:
            ...
            if target_parent is None:
                ...
                if target_parent is None:
                    guessed_group = _sales_report_guess_group(name)
                    if guessed_group:
                        target_parent = _find_root_group_id_by_name(cur, period_id, guessed_group)
                        if target_parent is not None:
                            _upsert_sales_report_rule(cur, store, name, user.get('username') or 'system', target_group_name=guessed_group, target_name=target_name, is_deleted=0)
            if target_parent is None:
    ...
    return RedirectResponse(_sales_report_redirect_url(store, month_key, import_ok=inserted), status_code=HTTP_303_SEE_OTHER)
...
async def sales_report_add_group(request: Request):
    ...
    if not name:
        return RedirectResponse(_sales_report_redirect_url(store, month_key), status_code=HTTP_303_SEE_OTHER)
    parent_id = int(parent_raw) if parent_raw.isdigit() else None
...
async def sales_report_move_group(request: Request, group_id: int):
    ...
    store, month_key, brand, _ = _sales_report_state(request, user, str(form.get('month_key') or ''), str(form.get('store') or ''))
    ph = _ph()
...
async def sales_report_rename_group(request: Request, group_id: int):
    ...
    store_state, month_state, _, _ = _sales_report_state(request, user, str(form.get('month_key') or ''), str(form.get('store') or ''))
    if not new_name:
        return RedirectResponse(_sales_report_redirect_url(store_state, month_state), status_code=HTTP_303_SEE_OTHER)
    ph = _ph()
    brand = _current_store_scope(request, user)
    with connect() as conn:
        cur = conn.cursor()
        ...
        conn.commit()
        request.session['sales_report_store'] = row.get('store')
        request.session['sales_report_month_key'] = row.get('month_key')
        return RedirectResponse(_sales_report_redirect_url(row.get('store'), row.get('month_key')), status_code=HTTP_303_SEE_OTHER)
